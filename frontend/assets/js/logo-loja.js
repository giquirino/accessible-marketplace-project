(function () {
  ...
  function mostrar(logo) {
    var tem = Boolean(logo);
    if (previa) { previa.src = logo || ''; previa.hidden = !tem; }
    if (icone) icone.hidden = tem;
    if (texto) texto.textContent = tem ? 'Clique ou arraste para trocar a logo' : TEXTO_PADRAO;
    if (botaoRemover) botaoRemover.hidden = !tem;
  }
  ...
  async function enviar(arquivo) {
    ...
    if (!Sola.logado()) throw new Error('Entre na sua conta para salvar a logo da loja.');

    var imagem = await carregarImagem(await lerArquivo(arquivo));
    var reduzida = recortarQuadrado(imagem);
    mostrar(reduzida);

    var loja = await Sola.api('/lojas/logo', { method: 'PUT', body: JSON.stringify({ logo: reduzida }) });
    mostrar(loja.logo);
  ...
  async function carregarLogoAtual() {
    if (!Sola.logado()) { mostrar(null); return; }
    try {
  ...
  campo.addEventListener('change', function () { processar(campo.files && campo.files[0]); });

  if (area) {
    area.addEventListener('click', function () { if (!campo.disabled) campo.click(); });

    ['dragenter', 'dragover'].forEach(function (evento) {
      area.addEventListener(evento, function (e) { e.preventDefault(); area.classList.add('caixa-envio-ativa'); });
    });
    ['dragleave', 'drop'].forEach(function (evento) {
      area.addEventListener(evento, function (e) { e.preventDefault(); area.classList.remove('caixa-envio-ativa'); });
    });
    area.addEventListener('drop', function (e) {
      processar(e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0]);
    });
  }
