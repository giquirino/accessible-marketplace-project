(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (formLogin) {
      ...
      if (pendente) {
        Sola.erroForm(formLogin, pendente);
        sessionStorage.removeItem('solaMensagemLogin');
      }
      ...
      formLogin.addEventListener('submit', async function (e) {
        ...
        if (!campoEmail.value || !campoSenha.value) {
          var campoComProblema = campoEmail;

          if (campoEmail.value) {
            campoComProblema = campoSenha;
          }

          Sola.erroForm(formLogin, 'Preencha o e-mail e a senha para entrar.', campoComProblema);
          return;
        }
        ...
        try {
          var resultado = await Sola.api('/auth/login', {
            method: 'POST',
    ...
    if (formCadastro) {
      formCadastro.addEventListener('submit', async function (e) {
        ...
        try {
          var resultado = await Sola.api('/auth/cadastro', {
            method: 'POST',
            body: JSON.stringify({
